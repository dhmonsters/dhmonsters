...
반복 검증으로 생긴 대용량 영상을 정리한다.
기본 보존 대상은 요약 보고서, 엑셀, 실패 contact sheet, 핵심 대표 프레임, 설정 스냅샷이다.
전체 영상은 최신 N개 세션과 사용자가 잠금 표시한 세션만 보존한다.
개선이 확정된 실험의 이전 영상은 폐기할 수 있지만, 폐기 전에 어떤 문제를 어떻게 개선했는지 짧은 보고서를 남긴다.
...
- 영상 보존 개수가 기준을 넘으면 잠금 없는 이전 영상은 정리 대상이다.
- 영상이 삭제되어도 개선 보고서와 핵심 실패 이미지만으로 변경 이력을 이해할 수 있어야 한다.
...
6. 영상 보존 정책과 자동 정리 흐름을 붙인다.
7. 10회 자동 검증으로 리포트가 정상 생성되는지 확인한다.
...
- 녹화 파일이 커질 수 있다.
  - 대응은 검증 모드에서 보존 개수, 잠금 세션, 자동 정리 정책을 둔다.
  - 영상은 임시 증거로 보고, 최종 기록은 간략 보고서와 핵심 이미지 중심으로 남긴다.
