...
chcp 65001 > nul
echo ========================================
echo  Claude v2.4.8 Build (PyArmor + PyInstaller + Recovery Launcher)
echo ========================================
...
if exist dist\Claude_2.4.8 rmdir /s /q dist\Claude_2.4.8
...
xcopy /E /I /Y .obf_build\dist\ClaudeApp dist\Claude_2.4.8
"%PYTHON%" build_release_config.py config.json dist\Claude_2.4.8\config.json
if errorlevel 1 (echo Release config generation failed & exit /b 1)
copy /Y version.txt dist\Claude_2.4.8\version.txt
if exist templates xcopy /E /I /Y templates dist\Claude_2.4.8\templates
if exist monsters xcopy /E /I /Y monsters dist\Claude_2.4.8\monsters
if exist maps xcopy /E /I /Y maps dist\Claude_2.4.8\maps
if exist "third_party\Interception-v1.0.1\Interception\library\x64\interception.dll" copy /Y "third_party\Interception-v1.0.1\Interception\library\x64\interception.dll" "dist\Claude_2.4.8\interception.dll"

call recovery_launcher\build_launcher.bat "dist\Claude_2.4.8"
if errorlevel 1 (echo Recovery launcher build failed & exit /b 1)
"%PYTHON%" release_bundle_validation.py ".obf_build\build\ClaudeApp\Analysis-00.toc" "dist\Claude_2.4.8"
if errorlevel 1 (echo Release native dependency validation failed & exit /b 1)

for /d /r "dist\Claude_2.4.8" %%D in (__pycache__) do @if exist "%%D" rmdir /s /q "%%D"
del /s /q "dist\Claude_2.4.8\*.pyc" >nul 2>&1

echo.
echo ========================================
echo  Done! dist\Claude_2.4.8\Claude.exe + ClaudeApp.exe
echo ========================================
