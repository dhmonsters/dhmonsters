...
chcp 65001 > nul
echo ========================================
echo  Claude v2.4.12 Build (PyArmor + PyInstaller + Recovery Launcher)
echo ========================================
...
if exist dist\Claude_2.4.12 rmdir /s /q dist\Claude_2.4.12
...
xcopy /E /I /Y .obf_build\dist\ClaudeApp dist\Claude_2.4.12
"%PYTHON%" build_release_config.py config.json dist\Claude_2.4.12\config.json
if errorlevel 1 (echo Release config generation failed & exit /b 1)
copy /Y version.txt dist\Claude_2.4.12\version.txt
if exist templates xcopy /E /I /Y templates dist\Claude_2.4.12\templates
if exist monsters xcopy /E /I /Y monsters dist\Claude_2.4.12\monsters
if exist maps xcopy /E /I /Y maps dist\Claude_2.4.12\maps
if exist "third_party\Interception-v1.0.1\Interception\library\x64\interception.dll" copy /Y "third_party\Interception-v1.0.1\Interception\library\x64\interception.dll" "dist\Claude_2.4.12\interception.dll"

call recovery_launcher\build_launcher.bat "dist\Claude_2.4.12"
if errorlevel 1 (echo Recovery launcher build failed & exit /b 1)
"%PYTHON%" release_bundle_validation.py ".obf_build\build\ClaudeApp\Analysis-00.toc" "dist\Claude_2.4.12" --startup-executable "dist\Claude_2.4.12\ClaudeApp.exe" --startup-argument=--release-startup-check --startup-timeout 30
if errorlevel 1 (echo Release native dependency validation failed & exit /b 1)

for /d /r "dist\Claude_2.4.12" %%D in (__pycache__) do @if exist "%%D" rmdir /s /q "%%D"
del /s /q "dist\Claude_2.4.12\*.pyc" >nul 2>&1

echo.
echo ========================================
echo  Done! dist\Claude_2.4.12\Claude.exe + ClaudeApp.exe
echo ========================================
