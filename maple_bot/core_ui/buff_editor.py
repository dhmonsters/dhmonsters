# 버프 목록과 버프 주기/누름 시간을 편집하는 UI 위젯입니다.
...
class BuffEditor(QWidget):
    """attack.normal_buffs 목록을 행 단위로 편집합니다."""
    ...
    def __init__(self, config, keys: tuple = ("attack", "normal_buffs")):
        ...
        self._rows.setSpacing(SPACING["xxs"])
    ...
    def _build_row(self, idx: int, b: dict) -> QWidget:
        ...
        cb = QCheckBox("사용")
        cb.setChecked(bool(b.get("enabled", True)))
        cb.toggled.connect(lambda v, i=idx: self.set_field(i, "enabled", bool(v)))
        h.addWidget(cb)

        key = QLineEdit(b.get("key", ""))
        key.setPlaceholderText("버프키")
        key.setFixedWidth(105)
        key.textChanged.connect(lambda v, i=idx: self.set_field(i, "key", v))
        h.addWidget(key)

        iv = QDoubleSpinBox()
        iv.setRange(0.01, 3600.0)
        iv.setDecimals(2)
        iv.setSingleStep(0.05)
        iv.setSuffix(" 초")
        iv.setFixedWidth(150)
