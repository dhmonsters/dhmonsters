...
class BlockEditor(QWidget):
    ...
    def _build_row(self, idx: int, blk: dict) -> QWidget:
        ...
        if blk["type"] == "move":
            ...
            top.addWidget(mt)
            sw = QSpinBox(); sw.setRange(1, 99); sw.setPrefix("왕복 "); sw.setFixedWidth(72)
            sw.setValue(int(blk.get("sweeps", 1)))
            sw.valueChanged.connect(lambda val, i=idx: self.set_field(i, "sweeps", val))
            top.addWidget(sw)

            s_sx = QSpinBox(); s_sx.setRange(0, 4000); s_sx.setPrefix("시작 "); s_sx.setFixedWidth(96)
            s_sx.setValue(int(blk.get("start_x", 0)))
            s_sx.valueChanged.connect(lambda val, i=idx: self.set_field(i, "start_x", val))
            bot.addWidget(s_sx)
            e_sx = QSpinBox(); e_sx.setRange(0, 4000); e_sx.setPrefix("끝 "); e_sx.setFixedWidth(96)
            ...
            top.addWidget(dr)
            es = QComboBox(); es.addItems(["left", "right"]); es.setFixedWidth(80)
