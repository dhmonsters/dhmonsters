# selector_shadow 로그를 읽어 기존 추적과 새 selector 판단 차이를 요약합니다.
...
def _shadow_record(frame: Mapping[str, object]) -> Mapping[str, object] | None:
    ...
    if not bool(record.get("available", False)):
        return record
    if _point(record.get("point")) is None:
        return record
    return record
...
def write_markdown_report(summaries: Iterable[Mapping[str, object]], out_path: str | Path) -> Path:
    ...
    if not items:
        lines.extend([
            ...
            "",
            "새 `planet_solver_noauth.py`로 녹화한 뒤 다시 실행하면 divergence 표가 생성됩니다.",
        ])
        out.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return out
    ...
    lines.extend([
        ...
        "",
        "| 파일 | 프레임 | shadow | divergence | recovery | less-jumpy | max-div | 주요 family |",
        "|---|---:|---:|---:|---:|---:|---:|---|",
    ])
    for item in items:
        ...
        if isinstance(families, Mapping) and families:
            ...
            top_family = "-"
        lines.append(
            "| {name} | {frames} | {shadow_frames} | {divergence_count} | "
            "{recovery_candidates} | {shadow_less_jumpy} | {max_divergence} | {family} |".format(
                name=item.get("name", ""),
                frames=item.get("frames", 0),
                shadow_frames=item.get("shadow_frames", 0),
                divergence_count=item.get("divergence_count", 0),
                recovery_candidates=item.get("recovery_candidates", 0),
                shadow_less_jumpy=item.get("shadow_less_jumpy", 0),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--out", default="03_output/2026-06-25_selector_shadow_analysis_v1.md")
