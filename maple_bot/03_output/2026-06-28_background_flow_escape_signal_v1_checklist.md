...
- [x] split/release event에서 배경 예상 위치에 남는 가지와 이탈하는 가지를 분리하는 테스트를 만든다.
- [x] box size가 커지는 순간의 두 방향 확장 후보를 feature로 기록한다.
- [x] 분리 후 몇 프레임 동안 background flow에서 벗어나는지 누적 점수화한다.
- [x] GT 16개에서 단독 신호와 기존 selector 결합 점수를 비교한다.
...
- 후보 박스 중심 기준 v1은 escape-only 0/16이었다.
- path 내부점 기준 v2는 escape-only 1/16이었다.
