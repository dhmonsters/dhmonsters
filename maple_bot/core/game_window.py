# 메인 프로그램과 독립 도구가 함께 사용하는 게임창 탐색 기능을 제공한다.
...
def get_game_client_rect_screen(
    hwnd: int,
    *,
    win32gui_module: Any | None = None,
...
def _candidate_from_hwnd(win32gui: Any, hwnd: int) -> GameWindowCandidate | None:
    ...
    if score is None:
        return None
    return GameWindowCandidate(hwnd, title, class_name, client_w, client_h, score)


def _title_candidate_from_hwnd(
    win32gui: Any,
    hwnd: int,
    keyword: str,
) -> GameWindowCandidate | None:
    try:
        if not win32gui.IsWindowVisible(hwnd):
            return None
        class_name = str(win32gui.GetClassName(hwnd))
