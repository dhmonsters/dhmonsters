...
**Architecture:** 접근 A — 이동은 기존 `BlockRunner`/`FloorHuntRunner` 스레드가, 공격은 메인 루프 `hunting_tick`이 담당한다. BlockRunner가 블록 실행 직전 `on_segment` 훅으로 "현재 사냥 구간 여부"를 런타임에 통지(`route_hunt_active`)하고, `hunting_tick`은 그 플래그가 켜져 있을 때만 `_monster_in_range()`→`combat.attack()`을 수행한다. 복귀용 층은 신규 순수함수 `floors_from_route()`가 move 블록 Y를 클러스터링해 만든다.
...
- `core/navigation/block_runner.py` (수정) — 생성자 `on_segment` 콜백 추가, `run_block` 진입 시 호출.
...
### Task 3: BlockRunner에 on_segment 훅 추가

**Files:**
- Modify: `core/navigation/block_runner.py:30-46` (생성자), `:78-79` (run_block 진입)

이동 모듈을 detector/combat에 의존시키지 않기 위해, BlockRunner는 "현재 실행 블록"만 콜백으로 통지한다.

- [ ] **Step 1: 생성자에 on_segment 인자 추가**
...
```python
        self._on_segment = on_segment   # callable(Block) | None — 블록 실행 직전 통지
```

- [ ] **Step 2: run_block 진입 시 훅 호출**
...
Expected: 기존 PASS 유지(on_segment 기본 None이라 동작 불변).

- [ ] **Step 4: Commit**

```bash
git add core/navigation/block_runner.py
git commit -m "feat(nav): BlockRunner on_segment 훅(블록 실행 직전 통지)"
```
...
def test_on_route_segment_sets_flag_for_hunt_and_pass():
    rt = _make_runtime()
    rt._on_route_segment(Block(type="move", start_x=10, end_x=50, mode="infinite"))
    assert rt._route_hunt_active is True
    rt._on_route_segment(Block(type="move", start_x=10, end_x=50, mode="count"))
    assert rt._route_hunt_active is True
    rt._on_route_segment(Block(type="move", start_x=10, end_x=50, mode="pass"))
    assert rt._route_hunt_active is False
    rt._on_route_segment(Block(type="ladder", ladder_x=30, y_top=50, y_bot=100))
    assert rt._route_hunt_active is False
...
Expected: FAIL — `AttributeError: 'BotRuntime' object has no attribute '_on_route_segment'`
...
```python
            ...
            on_segment=self._on_route_segment,
```

(c) `_monster_in_range` 메서드 정의 바로 위에 신규 메서드 추가:

```python
    def _on_route_segment(self, block) -> None:
        """루트 실행기가 블록 진입 시 호출 — 사냥 구간(move·pass아님)이면 공격 허용."""
