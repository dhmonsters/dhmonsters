...
- [ ] ⚠️ **캐릭터 위치 검출** — ★정정(2026-06-01 도면 99줄): 설계는 **C HSV+면적필터+`set_hsv_override`
      채택, A RGB 폐기**(A RGB가 "위치 미감지" 근본원인). 따라서 우리 HSV는 **설계대로 맞음**.
      이전 "A RGB로 포팅" 권고는 **설계 위반 오류 — 철회**. 실제 누락은 **C `set_hsv_override`(맵별 HSV 미세조정)**.
...
## 우선순위 제안 (영향 큰 순) — 2026-06-01 도면 채택결정에 맞춤

1. ⚠️ **방향키 keepalive**(40ms 재전송, A `_direction_keepalive_loop`) — 장거리 이동 끊김 방지.
2. ⚠️ **캐릭터 검출 `set_hsv_override`**(C, 맵별 HSV) — HSV는 유지(설계대로). 맵별 미세조정만 보강.
   + min_area/max_area·moments가 C와 동일한지 점검.
3. ⚠️ **위치 미감지 폴백**(A: 3초 후 방향전환) + 이탈 복귀를 도면(C 폐루프 도착확인)과 정합.
4. ⚠️ **안티밴(방지몹) 대응동작**(A anti_mob: 클릭/아이템) — 감지만 하고 대응 없음.
5. ⚠️ **자동판매 루프 연결**(A junk_seller + C 탭색/재시도), ❌ 마을귀환/사망감지/맵이탈.
6. ❌ 다운점프/포탈(B 흡수 예정), 점프후공격(A) — 동선/전투 확장.

> ※ 철회: "캐릭터 검출을 A RGB로 포팅" — 도면은 A RGB를 **폐기**하고 C HSV 채택. 우리 HSV가 정답.
> ※ 동선: 도면은 **C CoordScriptRunner(폐루프 도착확인)** 채택 — 우리 `_exec_move` TOLERANCE 루프가 그 모델.
> A `_do_climb`(도착확인 없는 2초 등반)은 회피 대상. (사다리 jump_grab은 보조로 유지하되 폐루프 확인 우선)
